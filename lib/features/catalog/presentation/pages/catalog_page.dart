...
class _CatalogPageState extends State<CatalogPage> {
  ...
  void initState() {
    super.initState();
    _controller = CatalogController(fabulariumPath: widget.fabulariumPath)
  ...
  void _onControllerChanged() {
    ...
    if (_selectedStudio != null &&
        !_controller.studios.any((studio) => studio.name == _selectedStudio)) {
  ...
  Future<void> _openModel(CatalogModel model, String studioName) async {
    await Navigator.of(context).push(
      MaterialPageRoute(
    ...
    // Model details can upload, clean or repair Telegram state. Refresh only
    // this model first, keeping the catalog responsive.
    await _controller.refreshTelegramStatus(model);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Fabularium Catalog'),
        actions: [
          IconButton(
            tooltip: 'Telegram',
            onPressed: () {
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                final studio = _controller.studios[index];
                return ListTile(
                  selected: _selectedStudio == studio.name,
                  ...
                  trailing: Text('${studio.models.length}'),
                  onTap: () => setState(() => _selectedStudio = studio.name),
                );
              },
            ),
          ),
  ...
  Widget _buildSearchContent() {
    ...
    final results = <(CatalogModel, String)>[];

    for (final studio in _controller.studios) {
      for (final model in studio.models) {
        ...
        if (_normalizeText(searchable).contains(query)) {
          results.add((model, studio.name));
        }
      }
    }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text('No models found for "$_searchQuery".'),
            const SizedBox(height: 16),
            OutlinedButton.icon(
              onPressed: _searchController.clear,
              icon: const Icon(Icons.clear),
    ...
    if (_selectedStudio == null) {
      return const Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.collections_bookmark_outlined, size: 72),
            SizedBox(height: 16),
            Text('Select a studio or search for a model'),
          ],
        ),
      );
    }

    final studio = _controller.studios.firstWhere(
      (studio) => studio.name == _selectedStudio,
    );

    return _buildGrid(
      title: studio.name,
      models: studio.models.map((model) => (model, studio.name)).toList(),
      showStudio: false,
    );
  }
  ...
  Widget _buildGrid({
    ...
    required List<(CatalogModel, String)> models,
          ...
          child: GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return CatalogModelCard(
                ...
                telegramStatus: _controller.telegramStatusFor(item.$1),
                onTap: () => _openModel(item.$1, item.$2),
              );
            },
          ),
        ),
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: _openPendingModels,
            icon: const Icon(Icons.add_box_outlined),
            label: const Text('Register Models'),
          ),
        ],
      ),
    );
