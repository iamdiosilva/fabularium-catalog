...
class TelegramStorageVerificationService {
  ...
  Future<TelegramStorageVerificationResult> verifyAndUpdate({
    ...
    TelegramStorageVerificationProgressCallback? onProgress,
  }) async {
    ...
    if (allPresent) {
      ...
      return TelegramStorageVerificationResult(
        journal: journal,
        allPresent: true,
        catalogExpected: catalogExpected.length,
        catalogFound: remote.catalogFound.length,
        filesExpected: filesExpected.length,
        filesFound: remote.filesFound.length,
        missingCatalogMessageIds: const <int>[],
        missingFilesMessageIds: const <int>[],
        structuralErrors: const <String>[],
      );
    }

    final error = _buildMissingError(
    ...
    return TelegramStorageVerificationResult(
      ...
      filesExpected: filesExpected.length,
      filesFound: remote.filesFound.length,
      missingCatalogMessageIds: missingCatalog,
      missingFilesMessageIds: missingFiles,
      structuralErrors: structuralErrors,
    );
  }

  Future<_TelegramStorageRemoteVerification> _verifyRemote({
    required TelegramStorageChannel catalogChannel,
    ...
    try {
      isolate = await Isolate.spawn<Map<String, dynamic>>(
        ...
        <String, dynamic>{
          ...
          'catalogChannelId': catalogChannel.id,
          'catalogAccessHash': catalogChannel.accessHash,
          'catalogMessageIds': catalogMessageIds,
          'filesChannelId': filesChannel.id,
          'filesAccessHash': filesChannel.accessHash,
          'filesMessageIds': filesMessageIds,
  ...
  void _report(
    TelegramStorageVerificationProgressCallback? callback, {
...
Future<void> _telegramStorageVerificationEntryPoint(
  ...
  final rawCatalogIds = bootstrap['catalogMessageIds'];
  final filesChannelId = bootstrap['filesChannelId'];
