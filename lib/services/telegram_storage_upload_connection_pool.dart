...
class TelegramStorageUploadConnectionPool {
  ...
  final List<tg.Client> _clients = <tg.Client>[];
  final List<TelegramSocket> _sockets = <TelegramSocket>[];
  bool _isOpening = false;
  bool _isClosed = false;

  TelegramStorageUploadConnectionPool({required this.telegramClient});

  List<tg.Client> get clients => List<tg.Client>.unmodifiable(_clients);
  int get size => _clients.length;

  Future<void> open({int size = 8}) async {
    if (_isClosed) {
    ...
    _isOpening = true;
    try {
      final mainClient = await telegramClient.connect();
      final savedDc = await _loadCurrentDataCenter();
      final dc = telegramClient.findDataCenter(savedDc.id) ?? savedDc;
      final operations = <Future<void>>[];
      for (int slot = 0; slot < size; slot++) {
        operations.add(
          _openSlot(mainClient: mainClient, dc: dc, slot: slot).then(
            (connection) async {
              if (_isClosed) {
              ...
              _sockets.add(connection.socket);
            },
          ),
        );
      }
      await Future.wait(operations);
      if (_clients.length != size) {
      ...
      await close();
      if (e is TelegramStorageUploadConnectionPoolException) rethrow;
      throw TelegramStorageUploadConnectionPoolException(
  ...
  Future<_TelegramStorageUploadConnection> _openSlot({
    ...
    try {
      ...
      client.stream.listen((_) {}, onError: (_) {});
      final response = await client
          .initConnection<t.Config>(
            apiId: TelegramConfig.apiId,
            deviceModel: 'Fabularium Catalog Upload V3 $slot',
  ...
  Future<t.DcOption> _loadCurrentDataCenter() async {
    final primary = File(_sessionFilePath());
    final legacy = File(p.join(Directory.current.path, 'telegram_auth.json'));
    File? file;
    ...
    final dc = Map<String, dynamic>.from(rawDc);
    final id = _readInt(dc['id']);
    final port = _readInt(dc['port']);
    final ipAddress = dc['ipAddress']?.toString().trim();
    if (id == null || port == null || ipAddress == null || ipAddress.isEmpty) {
      throw const TelegramStorageUploadConnectionPoolException(
  ...
  String _sessionFilePath() {
    ...
    return p.join(basePath, 'Fabularium', 'Telegram', 'telegram_auth.json');
  }
  ...
  tg.Client clientForPart(int partIndex) {
    if (_clients.isEmpty) {
      throw const TelegramStorageUploadConnectionPoolException(
        'Upload connection pool is not open.',
      );
    }
    return _clients[partIndex % _clients.length];
  }

  Future<void> close() async {
    if (_isClosed) return;
    _isClosed = true;
    final sockets = List<TelegramSocket>.from(_sockets);
    _clients.clear();
    _sockets.clear();
    await Future.wait(
      sockets.map((socket) async {
        try {
          await socket.close();
        } catch (_) {}
      }),
...
class TelegramStorageUploadConnectionPoolException implements Exception {
  final String message;
  const TelegramStorageUploadConnectionPoolException(this.message);
